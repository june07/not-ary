<template>
    <v-container class="h-100 d-flex justify-center" :class="{ 'my-0 py-0': smAndDown }" v-if="scantron">
        <v-sheet width="100%">
            ...
            <div class="wrong">
                <v-radio-group v-for="(wrongAnswer, id) of wrongAnswers" :index="id" :model-value="wrongAnswer" readonly>
                    <div class="d-flex align-center" v-for="option in getOptions(id)">
                        ...
                        <v-radio :class="{ 'mobile-radio': smAndDown }" :value="Object.values(option)[0]" color="red">
                            <template v-slot:label>
                                ...
                                <span v-else-if="Object.values(option)[0] === rightAnswer" class="font-weight-bold font-italic text-decoration-underline">{{ Object.values(option)[0] }}</span>
...
const getOptions = (id) => {
    ...
    rightAnswer.value = options.right
    return options ? [
        { right: options.right },
